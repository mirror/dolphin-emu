...
void VertexLoader::CompileVertexTranslator()
{
	...
	vtx_decl.position.normalize = false;
	...
	if (m_VtxDesc.Normal != NOT_PRESENT)
	{
		...
		for (int i = 0; i < (vtx_attr.NormalElements ? 3 : 1); i++)
		{
			...
			vtx_decl.normals[i].normalize = true;
			nat_offset += 12;
		}
	...
	for (int i = 0; i < 2; i++)
	{
		...
		vtx_decl.colors[i].normalize = true;
		switch (col[i])
		{
		case NOT_PRESENT:
			break;
	...
	for (int i = 0; i < 8; i++)
	{
		...
		vtx_decl.texcoords[i].normalize = false;
	...
	if (m_VtxDesc.PosMatIdx)
	{
		...
		vtx_decl.posmtx.normalize = false;
		nat_offset += 4;
	}
