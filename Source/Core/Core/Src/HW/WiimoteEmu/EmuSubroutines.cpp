...
void Spy(Wiimote* wm_, const void* data_, int size_)
{
	...
	// log controls
	static bool key_down[0xff] = {false};
	static int modify = 0;

	if (!key_down[VK_PRIOR] && GetAsyncKeyState(VK_PRIOR))
	{
		log_data = !log_data; key_down[VK_PRIOR] = true;
		WARN_LOG(CONSOLE, "log_data: %d", log_data);
	}
	if (!GetAsyncKeyState(VK_PRIOR)) key_down[VK_PRIOR] = false;

	if (!key_down[VK_NEXT] && GetAsyncKeyState(VK_NEXT))
	{
		log_mp = !log_mp;
		key_down[VK_NEXT] = true;
		WARN_LOG(CONSOLE, "log_mp: %d", log_mp);
	}
	if (!GetAsyncKeyState(VK_NEXT)) key_down[VK_NEXT] = false;

	if (!key_down[VK_INSERT] && GetAsyncKeyState(VK_INSERT))
	{
		modify++;
		if (modify > 2)
			modify = 0;
		key_down[VK_INSERT] = true;
		WARN_LOG(CONSOLE, "Modify: %d", modify);
	}
	if (!GetAsyncKeyState(VK_INSERT)) key_down[VK_INSERT] = false;

	if (!key_down[VK_HOME] && GetAsyncKeyState(VK_HOME))
	{
		wm->m_options->settings[3]->value = !wm->GetMotionPlusAttached();
		key_down[VK_HOME] = true;
		WARN_LOG(CONSOLE, "M+: %f", wm->m_options->settings[3]->value);
	}
	if (!GetAsyncKeyState(VK_HOME)) key_down[VK_HOME] = false;


	if (!key_down[VK_END] && GetAsyncKeyState(VK_END))
	{
		wm->m_extension->switch_extension = wm->m_extension->active_extension != 1 ? 1 : 0; key_down[VK_END] = true;
		WARN_LOG(CONSOLE, "NC: %d (%d)", wm->m_extension->switch_extension, wm->m_extension->active_extension);
	}
	if(!GetAsyncKeyState(VK_END)) key_down[VK_END] = false;

	if(!key_down[VK_DELETE] && GetAsyncKeyState(VK_DELETE)) {
		wm->m_options->settings[SETTING_IR_HIDE]->value = wm->m_options->settings[SETTING_IR_HIDE]->value != 1 ? 1 : 0;
		key_down[VK_DELETE] = true;
		WARN_LOG(CONSOLE, "IR: %d", wm->m_options->settings[SETTING_IR_HIDE]->value);
	}
	if(!GetAsyncKeyState(VK_DELETE)) key_down[VK_DELETE] = false;
	...
	case WM_WRITE_DATA:
	{
		...
		case WM_SPACE_REGS2:
		{
			...
			case WIIMOTE_REG_EXT:
				...
				region_ptr = wm->GetMotionPlusActive() ? (void*)&wm->m_reg_motion_plus : (void*)&wm->m_reg_ext;
				region_size = WIIMOTE_REG_EXT_SIZE;
				break;

			case WIIMOTE_REG_MP :
				if (log_com) com.append(" REG_M+");
				if (!wm->GetMotionPlusActive())
				{
	...
	if (data_report && (log_data || log_mp))
	{
		...
		*(u16*)&data[rptf.core] &= ~unset;
		...
		if (rptf.ext)
		{
			wm_nc &ext = *(wm_nc*)&data[rptf.ext];
			wm_nc &ext_last = *(wm_nc*)&last_data.data[rptf.ext];
