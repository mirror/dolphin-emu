...
void Spy(Wiimote* wm_, const void* data_, int size_)
{
	...
	// log all data reports
	bool log_data_all = false;
	...
	last_report_ &last_report = emu ? last_report_emu : last_report_real;
	...
	case WM_REQUEST_STATUS:
	{
		...
		if (log_com) com.append(StringFromFormat("WM_REQUEST_STATUS: rumble %d", status->rumble));
		break;
	}

	case WM_WRITE_DATA:
	{
		if (log_com) com.append("W");
		...
		if (log_com)
			com.append(StringFromFormat(" %02x %08x %02x: %s", wd->space, address, wd->size, ArrayToString(wd->data, wd->size, 0).c_str()));
	...
	case WM_READ_DATA:
	{
		...
		memcpy(&g_rd, rd, sizeof(wm_read_data));
		...
		if (log_com)
			com.append(StringFromFormat(" %02x %06x %04x", rd->space, address, rd->size));
	...
	case WM_SPEAKER_MUTE:
		if (log_com) com.append(StringFromFormat("WM_SPEAKER: mute: %d", sr->enable));
		break;
	...
	{
		if (log_com) com.append("R_REPLY");

		wm_read_data_reply* const rdr = (wm_read_data_reply*)(data + 2);
		u32 address = Common::swap16(rdr->address);
		u8 size = rdr->size + 1;
		...
		case WM_SPACE_REGS2:
		{
			u8 reg_type = Common::swap24(g_rd.address) >> 16;
			const u8 region_offset = (u8)address;
			void *region_ptr = NULL;
			int region_size = 0;
			...
			{
				formatted.append(StringFromFormat("extension ID: %s", ArrayToString(rdr->data, size).c_str()));
		...
		if (log_com)
			com.append(StringFromFormat(" %02x %04x %02x %02x: %s", g_rd.space, Common::swap24(g_rd.address), size, rdr->error, ArrayToString(rdr->data, size, 0).c_str()));

		// save key
		if (save && address == 0x40)
		{
	...
	default:
		LOG1(CONSOLE, "unknown channel 0x%02x", (emu ? "E" : "R"), sr->wm);
		break;
	}

	// call stack breakpoint point
	if (emu)
	{
		if (data_report)
		{
	...
	{
		string prefix = StringFromFormat("com[%s]", (emu ? "E" : "R"));
		switch(sr->wm)
		{
		case WM_WRITE_DATA:
		case WM_READ_DATA:
		case WM_READ_DATA_REPLY:
			LOG3(CONSOLE, "%s %s", prefix.c_str(), com.c_str());
			break;

		default:
			LOG2(CONSOLE, "%s %s", prefix.c_str(), com.c_str());
			break;
		}
	...
	if (data_report && (log_data || log_mp))
	{
		...
		string SCore = "", SAcc = "", SIR = "", SExt = "", SExtID = "";
		...
		// select log data
		string s_data = StringFromFormat("data[%s]"
			//" | id %s"
			" | %s"
			//" | c %s"
			//" | a %s"
		...
		bool dataReportChange = log_data_all
			|| !last_report.data
			|| report_mode != last_report.mode;
	...
	last_report.data = data_report;
	last_report.mode = report_mode;
#endif
}
