...
bool CWII_IPC_HLE_Device_di::IOCtl(u32 _CommandAddress) 
{
	...
	u32 Command			= Memory::IOS_Read_U32(BufferIn) >> 24;
	...
	case DVDLowOpenPartition:
		{
			...
			u64 const TMDOffset = ((u64)Memory::IOS_Read_U32(CommandBuffer.InBuffer[0].m_Address + 4) << 2 ) + 0x2c0;
...
{
	u32 Command = Memory::IOS_Read_U32(_BufferIn) >> 24;
	...
	if (_BufferOut != 0)
	{
		...
		// nonsense values
		Memory::IOS_Memset(_BufferOut, 0, _BufferOutSize);
	}
	...
	case DVDLowInquiry:
		{
			...
			Memory::IOS_Write_U32(0x00000002, _BufferOut);
			Memory::IOS_Write_U32(0x20060526, _BufferOut + 4);
			...
			Memory::IOS_Write_U32(0x41000000, _BufferOut + 8);
	...
	case DVDLowRead:
		{
			...
			u32 Size = Memory::IOS_Read_U32(_BufferIn + 0x04);
			u64 DVDAddress = (u64)Memory::IOS_Read_U32(_BufferIn + 0x08) << 2;
	...
	case DVDLowGetCoverReg:
		Memory::IOS_Write_U32(m_CoverStatus, _BufferOut);

		INFO_LOG(WII_IPC_DVD, "DVDLowGetCoverReg 0x%08x", Memory::IOS_Read_U32(_BufferOut));
		break;
	...
	case DVDLowGetCoverStatus:
		Memory::IOS_Write_U32(IsDiscInside() ? 2 : 1, _BufferOut);
	...
	case DVDLowUnencryptedRead:
		{
			...
			u32 DVDAddress32 = Memory::IOS_Read_U32(_BufferIn + 0x08);
	...
	case DVDLowSeek:
		{
			u64 DVDAddress = Memory::IOS_Read_U32(_BufferIn + 0x4) << 2;
			const char *pFilename = NULL;
			if (m_pFileSystem)
				pFilename = m_pFileSystem->GetFileName(DVDAddress);
			if (pFilename != NULL)
	...
	case DVDLowReadDiskBca:
		...
		Memory::IOS_Write_U32(1, _BufferOut + 0x30);
		break;
	...
	case DVDLowRequestError:
		...
		Memory::IOS_Write_U32(m_ErrorStatus, _BufferOut);
...
int CWII_IPC_HLE_Device_di::GetCmdDelay(u32 _CommandAddress)
{
	...
	case DVDLowUnencryptedRead:
	{
		u32 const Size = Memory::IOS_Read_U32(BufferIn + 0x04);
