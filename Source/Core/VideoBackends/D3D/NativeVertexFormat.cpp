...
DXGI_FORMAT VarToD3D(VarType t, int size, bool normalize)
{
	DXGI_FORMAT retval = DXGI_FORMAT_UNKNOWN;
	static const DXGI_FORMAT lookup1[5] = {
		DXGI_FORMAT_R8_UINT, DXGI_FORMAT_R8_SINT, DXGI_FORMAT_R16_UINT, DXGI_FORMAT_R16_SINT, DXGI_FORMAT_R32_FLOAT
	};
	static const DXGI_FORMAT lookup2[5] = {
		DXGI_FORMAT_R8G8_UINT, DXGI_FORMAT_R8G8_SINT, DXGI_FORMAT_R16G16_UINT, DXGI_FORMAT_R16G16_SINT, DXGI_FORMAT_R32G32_FLOAT
	};
	...
	static const DXGI_FORMAT lookup4[5] = {
		DXGI_FORMAT_R8G8B8A8_UINT, DXGI_FORMAT_R8G8B8A8_SINT, DXGI_FORMAT_R16G16B16A16_UINT, DXGI_FORMAT_R16G16B16A16_SINT, DXGI_FORMAT_R32G32B32A32_FLOAT
	};
	static const DXGI_FORMAT lookup1n[5] = {
		DXGI_FORMAT_R8_UNORM, DXGI_FORMAT_R8_SNORM, DXGI_FORMAT_R16_UNORM, DXGI_FORMAT_R16_SNORM, DXGI_FORMAT_R32_FLOAT
	};
	static const DXGI_FORMAT lookup2n[5] = {
		DXGI_FORMAT_R8G8_UNORM, DXGI_FORMAT_R8G8_SNORM, DXGI_FORMAT_R16G16_UNORM, DXGI_FORMAT_R16G16_SNORM, DXGI_FORMAT_R32G32_FLOAT
	};
	static const DXGI_FORMAT lookup3n[5] = {
		DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_UNKNOWN, DXGI_FORMAT_R32G32B32_FLOAT
	};
	static const DXGI_FORMAT lookup4n[5] = {
	...
	switch (size*2 + normalize)
	{
	case 2: retval = lookup1[t]; break;
	case 3: retval = lookup1n[t]; break;
	case 4: retval = lookup2[t]; break;
	case 5: retval = lookup2n[t]; break;
	case 6: retval = lookup3[t]; break;
	case 7: retval = lookup3n[t]; break;
	case 8: retval = lookup4[t]; break;
	case 9: retval = lookup4n[t]; break;
	default: break;
	}
...
void D3DVertexFormat::Initialize(const PortableVertexDeclaration &_vtx_decl)
{
	...
	if (format->enable)
	{
		...
		m_elems[m_num_elems].Format = VarToD3D(format->type, format->components, format->normalize);
	...
	for (int i = 0; i < 3; i++)
	{
		if (_vtx_decl.normal_offset[i] > 0)
		{
			...
			m_elems[m_num_elems].Format = VarToD3D(_vtx_decl.normal_gl_type, _vtx_decl.normal_gl_size, true);
			m_elems[m_num_elems].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
			++m_num_elems;
		}
	}
	...
	for (int i = 0; i < 2; i++)
	{
		if (_vtx_decl.color_offset[i] > 0)
		{
			...
			m_elems[m_num_elems].Format = VarToD3D(_vtx_decl.color_gl_type, 4, true);
			m_elems[m_num_elems].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
			++m_num_elems;
		}
	}
	...
	for (int i = 0; i < 8; i++)
	{
		if (_vtx_decl.texcoord_offset[i] > 0)
		{
			...
			m_elems[m_num_elems].Format = VarToD3D(_vtx_decl.texcoord_gl_type[i], _vtx_decl.texcoord_size[i], true);
			m_elems[m_num_elems].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
			++m_num_elems;
		}
	}
